...
class ArchiveQueries {
    ...
    */
    ...
    /**
    * Query to delete thread from antispam.
    *
    * @param int $threadId The ID of the thread to delete.
    * @param string $board The board uri where the thread is located.
    * @return void
    */
    public function deleteAntispamEntry(int $threadId, string $board): void {

        $query = $this->pdo->prepare('DELETE FROM `antispam` WHERE `board` = :board AND `thread` = :thread');
