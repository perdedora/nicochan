...
class IpNoteQueries {
	...
	public function getByIp(string $ip): array {
		...
		$query = $this->pdo->prepare(
			'SELECT ipn.*, `username` FROM `ip_notes` ipn
			 LEFT JOIN `mods` m ON ipn.`mod` = m.`id` 
			 WHERE ipn.`ip` = :ip ORDER BY ipn.`time` DESC'
		);
	 ...
	 * @return void
	 */
	public function add(string $ip, int $mod_id, string $body): void {
		$query = $this->pdo->prepare(
			'INSERT INTO `ip_notes` (`ip`, `mod`, `time`, `body`) VALUES (:ip, :mod, :time, :body)'
		);
		$query->bindValue(':ip', $ip);
