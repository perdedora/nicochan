...
class ShadowQueries {
	...
	* TODO: move this to antispam queries.
	*
	* @param string $board The board uri where the thread is located.
	* @param int $id The ID of the thread.
	* @param int $status New shadow status.
	*/
	public function updateAntispam(string $board, int $id, int $status): void {
		$query = $this->pdo->prepare(
			'UPDATE `antispam` SET `shadow` = :status WHERE `board` = :board AND `thread` = :thread'
		);
	...
	*/
	...
	/**
	* Query to delete delete antispam entry.
	* TODO: move this to antispam queries.
	*
	* @param int $id The ID of the thread.
	* @param string $board The board uri where the thread is located.
	*/
	public function deleteAntispam(int $id, string $board): void {
		$query = $this->pdo->prepare(
			'DELETE FROM `antispam` WHERE `shadow` = 1 AND `board` = :board AND `thread` = :thread'
		);
		$query->bindValue(':board', $board);
