	...
	private function matchCondition(
		...
		switch ($conditionLower) {
			...
			case 'ip':
				...
				if (!isset($post['ip_raw'])) {
					$this->logger->log(LogDriver::WARNING, "Post missing 'ip_raw' field for IP condition");
					return false;
				}

				return $this->checkRegex($value, $post['ip_raw'], 'IP');
			case 'rdns':
				$this->validateType($value, 'string', 'RDNS condition pattern');

				if (!isset($post['ip_raw'])) {
					$this->logger->log(LogDriver::WARNING, "Post missing 'ip_raw' field for RDNS condition");
					return false;
				}

				$hostname = \rDNS($post['ip_raw']);
				if ($hostname === $post['ip_raw']) {
					$this->logger->log(LogDriver::WARNING, "RDNS lookup failed for IP: {$post['ip_raw']}");
					return false;
				}
